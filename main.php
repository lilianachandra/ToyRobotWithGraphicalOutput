<html>
<input type="hidden" name="posisiX" id="posisiX">
<input type="hidden" name="posisiY" id="posisiY">
<input type="hidden" name="petunjuk" id="petunjuk">
...
$(function(){
    ...
    $.fn.PlaceRobot = function(positionX, positionY, direction){
        $currentElement = $(this);
		document.getElementById('posisiX').value = positionX;
		document.getElementById('posisiY').value = positionY;
		document.getElementById('petunjuk').value = direction;
        $currentElement.find('.element').each(function(i){
            if($(this).attr("data") == (positionY + "," + positionX)){
                for(x in DIRECTION){
                    if(x == direction){
                        $(this).html("<img src='robot.png' alt='robot-png' data='"+ positionX + "," + positionY + "," + direction + "'class='robot " +
...
function save(){
		var resultX = document.getElementById('posisiX').value;
		var resultY = document.getElementById('posisiY').value;
		var resultZ = document.getElementById('petunjuk').value
		window.location.href = 'save.php?datas='+resultX+','+resultY+','+resultZ;
}
</script>
</html>
